...
$$
\begin{array}[t]{@{}lcl@{}l@{}}
{\mathrm{allocmem}}(s, {\mathit{at}}~{}[ i .. {j^?} ]~{\mathit{share}}~\mathsf{page}) & = & (s \oplus \{ \mathsf{mems}~{\mathit{meminst}} \}, {|s{.}\mathsf{mems}|}) &  \\
 \multicolumn{4}{@{}l@{}}{\quad
\quad \mbox{if}~ {\mathit{meminst}} = \{ \mathsf{type}~({\mathit{at}}~{}[ i .. {j^?} ]~{\mathit{share}}~\mathsf{page}),\;\allowbreak \mathsf{bytes}~{(\mathtt{0x00})^{i \cdot 64 \, {\mathrm{Ki}}}} \}
} \\
\end{array}
$$
...
$$
\begin{array}[t]{@{}lcl@{}l@{}}
{\mathrm{instantiate}}(s, {\mathit{module}}, {{\mathit{externaddr}}^\ast}) & = & {s''''} ; \{ \mathsf{module}~{\mathit{moduleinst}} \} ; \begin{array}[t]{@{}l@{}} {(\mathsf{perform}~({{\mathit{act}}^?}))^{k}}~({\mathit{perform}}~{{\mathit{act}}_m^\ast}) \\
