...
(* Isabelle ASCII representations of unicode symbols *)
let ra = "\\<Rightarrow>"
let lra = "\\<Longrightarrow>"
let equiv = "\\<equiv>"
let conj = "\\<and>"
let disj = "\\<or>"
let limp = "\\<longrightarrow>"
let liff = "\\<longleftrightarrow>"
let neq = "\\<noteq>"
let leq = "\\<le>"
let geq = "\\<ge>"
let mem = "\\<in>"
...
and render_exp exp_type typids exp =
  ...
  | StrE fields -> lrec ^ " " ^ (String.concat ", " (List.map (fun (a, e) ->
    render_atom a ^ " = " ^ r_func e) fields)) ^ " " ^ rrec
  | DotE (e, a) ->
  ...
  | MemE (e1, e2) -> parens (r_func e1 ^ " " ^ mem ^ " set " ^ r_func e2)
...
and render_lambda quants text =
  parens (lam ^ " " ^ String.concat " " quants ^ ". " ^ text)
...
and render_path (paths : path list) typids typ at n name is_extend end_exp =
  let rec render_record_update t1 t2 t3 =
    ...
    | [t2] -> parens (t1 ^ " " ^ lrec ^ " " ^ t2 ^ " := " ^ t3 ^ "  " ^ rrec)
    | t2 :: q -> parens (t1 ^ " " ^ lrec ^ " " ^ t2 ^ " := " ^ render_record_update (t2 ^ " " ^ t1) q t3 ^ "  " ^ rrec)
  in
...
let render_record id quants fields =
  ...
  ^
    ...
    "\t\"append_" ^ id ^ inhabitance_quanters ^ " arg1 arg2 = " ^ lrec ^ "\n\t\t" ^
...
let exported_string =
  ...
  "\t\"list_alli_aux f n (x # q) = (f n x " ^ conj ^ " list_alli_aux f (Suc n) q)\"\n\n" ^
