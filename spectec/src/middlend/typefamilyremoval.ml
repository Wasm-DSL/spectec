...
let make_quant_set env quants = 
  List.fold_left (fun (env, bind_map) q -> 
    match q.it with 
    | ExpP (id, typ) -> (env, StringMap.add id.it typ bind_map)
    | DefP (id, params, typ) -> (Env.bind_def env id (params, typ, []), bind_map)
    | _ -> (env, bind_map)  
  ) (env, StringMap.empty) quants
