...
let generate_subst_list lhs binds rhs prems =
  ...
  let tfs = 
    List.concat_map (collect_bind tf_collector) binds @ 
  ...
  compute_cases binds subs

let t_clause clause =
  match clause.it with 
  | DefD (binds, lhs, rhs, prems) ->
    let subst_list = generate_subst_list lhs binds rhs prems in
...
let t_inst inst =
  ...
  | InstD (binds, lhs, deftyp) ->
    ...
    let subst_list = compute_cases binds subs in
    List.map (fun (binds', subst) -> 
      (* Subst all occurrences of the subE id *)
      let new_lhs = Il.Subst.subst_args subst lhs in
...
let t_rule rule = 
  ...
  List.map (fun (binds', subst) -> 
    ...
    let new_binds, _ = Il.Subst.subst_binds subst binds_filtered in
    (* Reduction is done here to remove subtyping expressions *)
    RuleD (id, new_binds, m, new_exp, new_prems) $ rule.at
  ) subst_list
...
let rec t_def def = 
  ...
  | RelD (id, mixop, typ, rules) ->
    { def with it = RelD (id, mixop, typ, List.concat_map t_rule rules) }
  | _ -> def
